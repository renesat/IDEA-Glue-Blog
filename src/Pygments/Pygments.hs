...
import           Text.Pandoc.Definition (Block(..), Pandoc, Format(..))
...
pygmentizeToHTML code options
         | not (null options) = T.pack <$> unixFilter "pygmentize" ["-l", map toLower (T.unpack (head options)),  "-f", "html"] (T.unpack code)
