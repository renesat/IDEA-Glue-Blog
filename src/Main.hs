...
blogPandocCompiler = pandocCompilerWithTransformM
...
feedCompiler renderer config ctx =
  renderer config ctx =<< fmap (take 10) . recentFirst =<< loadAllSnapshots
    "posts/*"
    "_content"
...
getFaviconPngs svgPath sizes = do
  tmpFiles <- sequence $ map
    ...
    sizes
  actions <- return $ foldr1 (++) $ map (uncurry createFaviconExportLine) $ zip
    sizes
    tmpFiles
